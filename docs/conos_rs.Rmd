...
* [Conos](https://github.com/hms-dbmi/conos) for the integration methods, and [SeuratWrappers](https://github.com/satijalab/seurat.wrappers) for the Conos &rarr; Seurat conversion. We also need [SeuratData](https://github.com/satijalab/seurat-data) to load in datasets for this vignette.
...
To learn more about this dataset, type `?broad` 

```{r broad, results='hide', cache=TRUE}
#InstallData("broad")
data("broad")
broad.panel <- SplitObject(broad, split.by = 'Method')
for(i in 1:length(broad.panel)) {
  broad.panel[[i]] <- NormalizeData(broad.panel[[i]], verbose = F) %>% FindVariableFeatures(verbose=F) %>% ScaleData(verbose=F) %>% RunPCA(verbose=F)
}
broad.con <- Conos$new(broad.panel)
broad.con$buildGraph(k=15, k.self=5, space='PCA', ncomps=30, n.odgenes=2000, matching.method='mNN', metric='angular', score.component.variance=TRUE, verbose=TRUE)
...
```{r immune_stim, results='hide', cache=TRUE}
#InstallData("immune")
data("immune")
immune.panel <- SplitObject(immune, split.by = 'stim')
for(i in 1:length(immune.panel)) {
  immune.panel[[i]] <- NormalizeData(immune.panel[[i]], verbose = F) %>% FindVariableFeatures(verbose=F) %>% ScaleData(verbose=F) %>% RunPCA(verbose=F)
}
immune.con <- Conos$new(immune.panel)
immune.con$buildGraph(k=15, k.self=5, space='PCA', ncomps=30, n.odgenes=2000, matching.method='mNN', metric='angular', score.component.variance=TRUE, verbose=TRUE)
...
To learn more about this dataset, type `?panc` 
