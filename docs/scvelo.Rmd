...
#   }
#   system2(command = pip3, args = c("install", "--user", "scvelo"))
# }
```

```{python install_py_deps}
try:
  import scvelo
except ImportError:
  import pip
  pip.main(["install", "--user", "scvelo"])
...
```{r download, results="hide", echo=FALSE, eval=FALSE}
...
```{r load_data3, results='hide', eval=TRUE, eval=FALSE}
...
```{python scvelo, results="hide", eval=FALSE}
...
```{python latent_time, results="hide", eval=FALSE}
