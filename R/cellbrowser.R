...
ExportToCellbrowser <- function(
  ...
  message("Seurat Version installed: ", packageVersion(pkg = "Seurat"))
  object.version <- as.character(x = slot(object = object, name = 'version'))
  message("Object was created with Seurat version ", object.version)
  if (substr(x = object.version, start = 1, stop = 1) != '2' && substr(x = object.version, start = 1, stop = 1) != '3') {
    stop("can only process Seurat2 or Seurat3 objects, version of rds is ", object.version)
  }
  if (substr(x = object.version, start = 1, stop = 1) != substr(x = packageVersion(pkg = "Seurat"), start = 1, stop = 1)) {
  ...
  if (substr(x = object.version, start = 1, stop = 1) == '2') {
  ...
  if (!skip.expr.matrix) {
    if (use.mtx) {
      ...
      write(x = colnames(x = counts), file = barcodesPath)
      ...
      if ((((ncol(x = counts) / 1000) * (nrow(x = counts) / 1000)) > 2000) && is(object = counts, 'sparseMatrix')) {
        writeSparseMatrix(inMat = counts, outFname = gzPath)
      } else {
        mat <- as.matrix(x = counts)
        df <- as.data.frame(x = mat, check.names = FALSE)
        df <- data.frame(gene = genes, df, check.names = FALSE)
        z <- gzfile(description = gzPath, open = "w")
  ...
  for (embedding in embeddings) {
    ...
    embedNames <- append(x = embedNames, values = embedding)
  }
  ...
  embedNames <- sort(embedNames, decreasing = TRUE)
  embeddings.conf <- c()
